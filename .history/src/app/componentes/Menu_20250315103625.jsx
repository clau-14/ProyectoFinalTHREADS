...
const handleLogout = () => {
  ...
  signOut(auth)
    .then(() => {
      ...
      router.push("/src/app/componentes/Iniciosesion.jsx"); // Redirigir a la página de inicio de sesión
    })
    .catch((error) => {
...
const confirmLogout = () => {
  ...
  <CgDetailsMore className="relative hover:text-black w-12 h-12 flex items-center justify-center"
  onClick={() => {
    if (window.confirm("¿Estás seguro de que quieres cerrar sesión?")) {
      const auth = getAuth(); // Asegúrate de importar esto al inicio del archivo
      signOut(auth)
        .then(() => {
          console.log("Sesión cerrada exitosamente.");
          router.push("/inicio-sesion"); // Redirige a la página de inicio de sesión
        })
        .catch((error) => {
          console.error("Error al cerrar sesión:", error);
        });
