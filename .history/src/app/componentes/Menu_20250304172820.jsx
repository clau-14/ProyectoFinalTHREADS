...
const Menu = ({ handleOpenModal }) => {
  ...
  return (
    ...
    <ul className="flex flex-col items-center justify-center gap-9 ">
      ...
      <li className= {`mt-11 ${activeIcon === 'home' ? 'text-black' : 'hover:bg-gray-200 hover:p-1 hover:text-black active:bg-gray-300'}`}
          onClick={() => handleIconClick('home')}> 
        <AiFillHome /> {/* Icono para ir a la página de inicio */}
      </li>
      <li className={`${activeIcon === 'search' ? 'text-black' : 'hover:bg-gray-200 hover:text-black active:bg-gray-300'}`}
     ...
     <BiPlus /> {/* Icono para agregar una nueva publicación */}
      ...
      <li className={`${activeIcon === 'like' ? 'text-black' : 'hover:bg-gray-200 hover:text-black active:bg-gray-300'}`}
          onClick={() => handleIconClick('like')}
        >
      <img src="/like.png" alt="" />{/* icono para dar me gusta */}
      </li>
      <li className={`-mt-7 ${activeIcon === 'user' ? 'text-black' : 'hover:bg-gray-200 hover:text-black active:bg-gray-300'}`}
          onClick={() => handleIconClick('user')}>
        <HiOutlineUser />
      </li>
      </ul>
      <ul className="mt-32 flex flex-col items-center justify-center gap-5"> 
      <li className={`${activeIcon === 'pin' ? 'text-black' : 'hover:bg-gray-200 hover:text-black active:bg-gray-300'}`}
          onClick={() => handleIconClick('pin')}>
        <MdOutlinePushPin />
        
      </li>
      <li className={`${activeIcon === 'details' ? 'text-black' : 'hover:bg-gray-200 hover:text-black active:bg-gray-300'}`}
