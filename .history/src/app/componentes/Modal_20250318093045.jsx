...
const Modal = ({ show, onClose, onPublish, editingPost  }) => {
  ...
  thread.content.trim() !== "" || thread.files.length > 0 ? ( // Mostrar solo si hay contenido
    <section key={thread.id} className="thread-container flex flex-col mb-4">
      <div className="flex items-center">
        ...
        />
        <h1 className="ml-4 font-bold">{user.displayName}</h1>
      </div>
      <textarea
      ...
      <div className="mt-2">
        {thread.files.map((file, fileIndex) => (
          <div key={fileIndex} className="relative">
            {file.type === "image" && <img src={file.preview} alt={file.name} className="rounded-md w-full" />}
            {file.type === "video" && <video src={file.preview} controls className="rounded-md w-full" />}
            <button
