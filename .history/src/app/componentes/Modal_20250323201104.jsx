...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handleAddPost = async (content, userId) => {
    if (!userId || !content || !content.some((thread) => thread.text.trim() !== "")) {
      console.error("Error: userId o contenido inválido.");
      console.log("Contenido recibido:", content);
      console.log("userId recibido:", userId);
      return; // Detenemos el flujo si no hay datos válidos
    }
    ...
    try {
      const newPost = {
        content,
        userId,
        timestamp: new Date(), // Fecha de creación
        likes: [], // Inicializamos con likes vacíos
      };
  
      console.log("Datos enviados a Firestore:", newPost);
      await createPost(newPost); // Guardamos en Firestore
