...
const Modal = ({ show, onClose, onPublish, editingPost }) => {
  ...
  const handleInput = (index, event) => {
    ...
    const hasContent = newThreads.some((thread) => thread.content.trim() !== "" || thread.files.length > 0);
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-auto">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
        ...
        {threads.map((thread, index) => (
          <section key={thread.id} className="flex flex-col mb-4">
            ...
            <div className="mt-2">
              {thread.files.map((file, fileIndex) => (
                <div key={fileIndex} className="relative">
                  {file.type === "image" && (
                    <img src={file.preview} alt={file.name} className="rounded-md w-full" />
                  )}
                  {file.type === "video" && (
                    <video src={file.preview} className="rounded-md w-full" controls />
                  )}
                  <button
