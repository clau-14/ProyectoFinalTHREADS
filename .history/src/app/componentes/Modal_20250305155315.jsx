...
const Modal = ({ show, onClose }) => {
  ...
  const [threads, setThreads] = useState([
    ...
    { id: 2, active: false, placeholder: "¿Di algo más?", showUser: false } // segundo hilo inactivo
  ]);
  ...
  const handleRemoveThread = (index) => {
    let newThreads = threads.filter((_, i) => i !== index); // Elimina el hilo específico
    if (newThreads.length === 1) {
      newThreads = [
        { ...newThreads[0], active: false, showUser: false }, // Desactiva el segundo hilo si es el único que queda
        { id: 1, active: true, placeholder: "¿Qué novedades tienes hoy?", showUser: true }, // Asegura que el primer hilo siempre esté presente y activado
        { id: 2, active: false, placeholder: "¿Di algo más?", showUser: false } // Asegura que el segundo hilo siempre esté presente y desactivado
      ];
          }
    setThreads(newThreads);
  };
