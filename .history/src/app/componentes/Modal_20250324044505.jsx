...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handlePublish = async () => {
    ...
    try {
      if (editingPost) {
        ...
        console.log("Actualizando publicación con ID:", editingPost.id);
      ...
      console.error("Error durante el flujo de publicación:", error);
    }
  };
  ...
  const handleAddPost = async (content, userId) => {
    ...
    try {
      ...
      await createPost(newPost); // Guardar nueva publicación en Firestore
  ...
  useEffect(() => {
    if (editingPost) {
      ...
      console.error("editingPost no contiene datos válidos.");
    }
  }, [editingPost]);
