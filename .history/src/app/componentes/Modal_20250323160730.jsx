...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handlePublish = async () => {
    if (!user?.uid) {
      console.error("Error: Usuario no autenticado. No se puede publicar.");
      return;
    }
  
    const content = threads.map((thread) => ({
      text: thread.content,
      files: thread.files, // Archivos seleccionados
    }));
    ...
    if (editingPost) {
      ...
      await onAddPost(content, user.uid, threads[0]?.files || []);
  ...
  const handlePublish = async () => {
    // Mapeo del contenido de los hilos para preparar los datos
    const content = threads.map((thread) => ({
      text: thread.content,
      files: thread.files, // Archivos seleccionados
    }));
    ...
    if (editingPost) {
      // Editar publicación existente
      await onPublish(content); // Aquí pasamos los datos para la edición
      console.log("Publicación editada:", content);
    } else {
      // Crear nueva publicación
      await onAddPost(content, user?.uid, threads[0]?.files || []);
