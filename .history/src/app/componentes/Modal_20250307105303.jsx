...
const Modal = ({ show, onClose }) => {
  ...
  const handleRemoveThread = (index) => {
    const newThreads = [...threads];
    if (index === 1) {
      newThreads[index] = { id: 2, active: false, placeholder: " ", showUser: false, content: "", ModalBotones }; // Desactiva y borra el contenido del segundo hilo
    } else {
        ...
        {threads.map((thread, index) => (
          <section key={thread.id} className={`flex-1 ${thread.active ? '' : 'opacity-50 pointer-events-none'}`}>
            ...
            ></textarea>
             ...
             <ModalBotones onButtonClick={handleButtonClick} />
          </section>
        ))}
