...
import { createPost } from "@/firebase/crud"; //desactivado
import { updatePost } from "@/firebase/crud"; //desactivado


const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => { //onAddPost desactivado
  if (!show) return null;

  const { user } = useUserStore();
  const router = useRouter();
  const [content, setContent] = useState(""); //desactivado
  ...
  const handlePublish = async () => {
    const content = threads.map((thread) => ({
      text: thread.content.trim(), // Asegúrate de que el texto esté limpio
      files: thread.files || [], // Mantenemos un array incluso si no hay archivos
    }));
  
    console.log("Contenido preparado para enviar a onPublish:", content);
  
    try {
      if (!content.length) {
        console.error("Error: No hay contenido para publicar.");
        return;
      }
      ...
      if (editingPost) {
        ...
        if (!editingPost.id) {
          console.error("Error: El ID de la publicación para editar está vacío.");
          return;
        }
  
        console.log("Editando publicación con ID:", editingPost.id);
        await updatePost(editingPost.id, { content }); // Guardar en Firebase
        console.log("Publicación editada exitosamente en la base de datos.");
      } else {
        ...
        const newPost = {
          ...
          userId: user.uid,
          timestamp: new Date(),
          likes: [], // Campo opcional para "Me gusta"
        };
  
        console.log("Creando nueva publicación:", newPost);
        await createPost(newPost); // Guardar en Firebase
        console.log("Nueva publicación creada exitosamente en la base de datos.");
      }
  
      // Mantener la funcionalidad de onPublish
      if (onPublish) {
        onPublish(content); // Pasa los datos de vuelta al padre si es necesario
      }
      ...
      console.error("Error durante la publicación en Firebase:", error);
    }
  };
  ...
  useEffect(() => {
    if (editingPost) {
      console.log("Datos de editingPost recibidos:", editingPost);
  ...
  const handleInput = (index, event) => {
    ...
    const hasContent = newThreads.some((thread) => thread.content !== "");
