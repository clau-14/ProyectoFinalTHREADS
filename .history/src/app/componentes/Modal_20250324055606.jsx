...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => { //onAddPost desactivado
  ...
  const handlePublish = async () => {
    ...
    try {
      // Subir archivos antes de guardar la publicación
      for (const thread of content) {
        if (thread.files.length) {
          thread.files = await Promise.all(
            thread.files.map((file) => uploadFile(file))
          );
        }
      }
      ...
      if (editingPost) {
        ...
        console.log("Publicación editada:", content);
      } else {
        const newPost = {
          content,
          userId: user.uid,
