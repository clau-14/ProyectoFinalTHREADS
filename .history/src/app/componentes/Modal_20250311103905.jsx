...
const Modal = ({ show, onClose, onPublish }) => {
  ...
  const handleRemoveThread = (index) => { 
    ...
    { newThreads.splice(index, 1); // Elimina el hilo específico if (newThreads.length > 1) { newThreads[newThreads.length - 1].placeholder = ""; // Elimina el archivo del array currentThread.files.splice(fileIndex, 1); } } setThreads(newThreads); };
