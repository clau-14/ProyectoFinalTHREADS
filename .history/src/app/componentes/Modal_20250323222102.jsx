...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handleAddPost = async (content, userId) => {
    await createPost(content, userId); // Llama a la función CRUD
    ...
    // Aquí puedes actualizar el estado de la página para reflejar la nueva publicación
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      if (editingPost) {
        // Caso de edición: actualiza la publicación existente
        ...
        // Caso de creación: crea una nueva publicación
  ...
  useEffect(() => {
    console.log("Datos de editingPost:", editingPost); // Log para depuración
    if (editingPost) {
      ...
      setIsPublishEnabled(true); // Habilitar el botón "Publicar"
    } else {
      setThreads([{ id: 1, active: true, placeholder: "¿Qué novedades tienes hoy?", content: "", files: [] }]);
    }
  }, [editingPost]);
  ...
  const handleInput = (index, event) => {
    ...
    // Verifica si algún hilo tiene contenido para habilitar el botón "Publicar"
    const hasContent = newThreads.some((thread) => thread.content.trim() !== "");
    setIsPublishEnabled(hasContent); // Esto controla el estado del botón
  };
