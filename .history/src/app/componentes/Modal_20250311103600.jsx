...
const Modal = ({ show, onClose, onPublish }) => {
  ...
  const handleRemoveThread = (index) => { const newThreads = [...threads]; if (index === 1) { newThreads[index] = { id: 2, active: false, placeholder: " ", showUser: false, content: ""}; // Desactiva y borra el contenido del segundo hilo

  // Elimina un hilo
  const handleRemoveThread = (threadIndex) => {
    const updatedThreads = [...threads];
    updatedThreads.splice(threadIndex, 1); // Elimina el hilo
