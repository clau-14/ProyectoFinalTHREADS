...
import { updatePost } from "@/firebase/crud"; 
...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => { //onAddPost desactivado
  ...
  const createNewPost = async (content, userId) => {
    const newPost = {
      content,
      userId,
      timestamp: new Date(),
    ...
    try {
      console.log("Creando nueva publicación en Firebase:", newPost);
      await createPost(newPost);
      console.log("Nueva publicación creada exitosamente.");
    } catch (error) {
      console.error("Error al crear la publicación:", error);
      throw error; // Lanza el error para manejarlo donde sea invocado
    }
  };
  const editExistingPost = async (postId, content) => {
    try {
      console.log("Editando publicación en Firebase con ID:", postId);
      await updatePost(postId, { content });
      console.log("Publicación editada exitosamente en Firebase.");
    } catch (error) {
      console.error("Error al editar la publicación:", error);
      throw error; // Lanza el error para manejarlo donde sea invocado
    }
  };
  const saveToDatabase = async (content, editingPost, user) => {
    if (editingPost) {
  ...
  const handlePublish = () => {
    ...
    if (onPublish) {
      onPublish(content); // Llama a la función pasada como prop
    } else {
      console.error("Error: onPublish no está definido.");
    }
  
    // Llama a la función externa para guardar en la base de datos
    saveToDatabase(content, editingPost, user);
  };
