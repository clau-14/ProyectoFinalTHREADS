...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handleAddPost = async (content, userId) => {
    if (!userId) {
      console.error("Error: userId no está definido. Deteniendo operación.");
      return;
    }
  
    try {
      const newPost = {
        content: content || "Sin contenido", // Aseguramos que siempre haya contenido
        userId, // Debe ser un valor válido
        timestamp: new Date(),
        likes: [], // Inicializamos los likes
      };
  
      console.log("Datos listos para guardar en Firestore:", newPost);
      await createPost(newPost); // Intentamos guardar los datos
