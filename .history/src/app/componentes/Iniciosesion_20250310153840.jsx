...
const Iniciosesion = () => {
  ...
  const fetchInstagramUserData = async () => { 
    const accessToken ="EAATJPntzLx4BO74UVYY6sgcDJ3wEf6DhZAe2LKgwZB7x1MthM4tHhen44dEuMYaCjZB7Y6kSHkLsEJ5tBx3b1sYxO7LRALOaraD0cOnUGZBtP5cKZAk4ndWfI1G5zxsVTBstXBIZAKcZBimIyvzjMnahLH03DJoX9vD9wR8khzzUjwuspiFjeas"
    const response = await fetch(`https://graph.instagram.com/me?fields=id,username&acss_token=${accessToken}`);
    const data = await response.json();
  
    if (response.error) {
      console.log("Error al obtener datos de Instagram:", response.error.message);
    } else {
      console.log("Usuario de Instagram:", data);
    }
  };
  ...
  const responseInstagram = (respuesta) => {
    if (respuesta.error_type) {
      ...
      console.log("Token de Instagram recibido:", respuesta.access_token);
      fetchInstagramUserData(respuesta.access_token); // Obtén datos del usuario
      alert("¡Éxito!");
      iniciarSesion2(respuesta.user);
      router.push("/pagina1");
    }
  ...
  */
  ...
  return (
    <div className="container sm:w-full md:w-full lg:w-full xl:w-full flex flex-col items-center justify-center -m-10">
      ...
      <InstagramLogin
  clientId="502427716027599"
  redirectUri="http://localhost/pagina1" // Cambia esto según tu configuración
