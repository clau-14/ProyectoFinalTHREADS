...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => { //onAddPost desactivado
  ...
  const handlePublish = async () => {
    ...
    console.log("Contenido preparado para publicar:", content);
    ...
    try {
      ...
      if (editingPost) {
        // Verificar que `editingPost` tiene un ID válido
        if (!editingPost.id) {
          throw new Error("El ID de la publicación para editar está vacío.");
        }
  
        console.log("Intentando editar publicación con ID:", editingPost.id);
        await updatePost(editingPost.id, { content }); // Guardar cambios en Firebase
        console.log("Publicación editada exitosamente:", content);
      } else {
        ...
        const newPost = {
          ...
          userId: user.uid,
          timestamp: new Date(),
          likes: [], // Campo opcional
        };
  
        console.log("Intentando crear nueva publicación:", newPost);
        await createPost(newPost); // Guardar nueva publicación en Firebase
        console.log("Nueva publicación creada exitosamente:", newPost);
      }
  
      // Funcionalidad de onPublish
      if (onPublish) {
        console.log("Pasando datos al padre vía onPublish.");
        onPublish(content); // Notificar al componente padre
      }
