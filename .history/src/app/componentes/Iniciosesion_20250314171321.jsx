...
const Iniciosesion = () => {
  ...
  const { iniciarSesion2 } = useUserStore();
   ...
   const iniciaSeccion = async (email, password) => {
    try {
      const result = await signInWithEmailAndPassword(auth, email, password);
  
      // Validar que la cuenta es de Google (opcional, ya que Firebase lo gestiona)
      if (!email.endsWith("gmail.com")) {
        throw new Error("Este correo no es de Google.");
      }
  
      console.log("Autenticado con email:", result.user);
      return result.user;
    } catch (error) {
