...
const Modal = ({ show, onClose }) => {
  ...
  <div
    ...
    <section className="">
    <label className="flex items-center gap-4 -mb-6 pb-10 "><img className='rounded-full w-10  -ml-2 mt-2 z-40' src={user ? user.photoURL  : null} alt="" /> <botton>Agregar</button> <button className="" onClick={onClose}>X</button></label>
    <textarea ref={textareaRef} className="w-full border-l-2 ml-3 pl-6 -pt-0.5 -mt-4 outline-none focus:ring-0
     " type="text" placeholder="¿Que novedases tienes?" rows="1"
     onInput={handleInput} style={{ overflow: 'hidden', resize: 'none' }}></textarea>
     </section>
