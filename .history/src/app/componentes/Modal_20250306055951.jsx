        ...
        {threads.map((thread, index) => (
         ...
         <section className="flex justify-between items-center pt-4">
          <div className="flex items-center gap-2">
            <span  className={`px-4 py-2 rounded-lg ${isPublishEnabled ? ' text-black border-2 border-gray-200 rounded-lg font-bold' : 'text-grey-300 border-1 text-gray-400 '}`}>Cualquiera puede responder o citar</span>
          </div>
          <button
            className={`px-4 py-2 rounded-lg ${isPublishEnabled ? ' text-black border-2 border-gray-200 rounded-lg font-bold' : 'text-grey-300 border-1 text-gray-400 '}`}
            onClick={handlePublish}
            disabled={!isPublishEnabled}border-1 border-gray-400 rounded-lg
          >
            Publicar
          </button>
        </section>
