  ...
  const handleInput = (index, event) => {
    ...
    useEffect(() => {
      if (Array.isArray(defaultValue) && defaultValue.length > 0) {
        // Mapear los datos de defaultValue al formato de los threads
        const mappedThreads = defaultValue.map((item, index) => ({
          id: index + 1,
          active: true,
          placeholder: index === 0 ? "¿Qué novedades tienes hoy?" : "¿Di algo más?",
          content: item.text || "", // Asegura que haya contenido
          files: item.files || [] // Asegura que haya archivos o un array vacío
        }));
        setThreads(mappedThreads); // Actualiza el estado de threads con los datos existentes
      } else {
        // Si no hay defaultValue, usa threads por defecto
        setThreads([
          { id: 1, active: true, placeholder: "¿Qué novedades tienes hoy?", content: "", files: [] },
