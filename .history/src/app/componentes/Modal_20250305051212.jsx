    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-auto">
      <div
        ...
        <label className="flex items-center gap-4 mb-4">
          <img className='rounded-full w-10 mt-2' src={user ? user.photoURL : null} alt="" />
          <h1 className="">{user ? user.displayName : null}</h1>
        </label>
        <textarea
...
export default Modal;  className="w-full  border-none outline-none focus:ring-0" type="text" placeholder="¿Que novedases tienes?" />
  </div>
</div>
  );
};
