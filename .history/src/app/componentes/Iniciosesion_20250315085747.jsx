...
import { auth } from "@/firebase/firebaseConfig";import { GoogleAuthProvider, signInWithEmailAndPassword, signInWithPopup } from 'firebase/auth';
...
const Iniciosesion = () => {
  ...
  const iniciaGoogle = async () => {
    ...
    try {
      ...
      toast.success("¡Inicio de sesion Éxitoso!");
  ...
  return (
    <div className="container sm:w-full md:w-full lg:w-full xl:w-full flex flex-col items-center justify-center -m-10">
       <Toaster position="top-center" reverseOrder={false} toastOptions={{ duration: 10000, style: {background: '#333', color: '#fff',},}}/>
      ...
      <form className='w-full flex flex-col items-center justify-center md:-mt-40 lg:-mt-40 xl:-mt-40' onSubmit={iniciaSeccion}>
        ...
        <div className="relative w-3/4 md:w-1/4 lg:w-1/4 xl:w-1/4 mt-8 text-center font-bold text-black text-sm py-8 font-sans rounded-xl border border-solid border-zinc-300" onClick={iniciaGoogle}>
          <h1 className="-ml-10">Continuar con Google<span className="absolute left-3 top-5 text-5xl"><FcGoogle /></span></h1>
      ...
      <footer className="flex relative justify-center w-full text-gray-400 text-xs mt-16 sm-text-xs44">
        ...
        <section className='hidden  md:block lg:block xl:block'>
          <p className='absolute -mt-60 ml-52'>Escanea el código para descargar la app</p>
          <div className=''><img className='absolute -mt-52 ml-60 p-4 rounded-lg shadow-md transform transition duration-300 hover:scale-110 hover:shadow-lg' src="/qr.png" alt="" /></div>
        </section>
      </footer>
    </div>
  );
