...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handleAddPost = async (content, userId) => {
    ...
    try {
      ...
      const handlePublish = async () => {
        ...
        try {
          if (editingPost) {
            console.log("ID de la publicación a editar:", editingPost.id);
            console.log("Datos a actualizar:", { content });
            
            // Llama a `updatePost` y pasa los parámetros necesarios
            await updatePost(editingPost.id, { content }); 
            console.log("Publicación editada exitosamente:", content);
          } else {
            await onAddPost(content, user.uid); // Crea una nueva publicación
  ...
  useEffect(() => {
    console.log("Datos de editingPost:", editingPost); // Log para depurar
    if (editingPost) {
