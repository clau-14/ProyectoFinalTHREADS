...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handlePublish = async () => {
    if (!user?.uid) {
      ...
      return; // Detenemos el flujo si `user?.uid` no está disponible
    }
  
    const content = threads.map((thread) => ({
      text: thread.content.trim(), // Aseguramos que no sea solo espacios
      files: thread.files || [], // Incluimos archivos seleccionados
    }));
  
    if (!content.length || content.every((thread) => !thread.text)) {
      console.error("Error: No hay contenido válido para publicar.");
      return; // Detenemos el flujo si no hay contenido válido
    }
  
    // Aquí llamamos directamente a `handleAddPost`
    await handleAddPost(content, user.uid); 
    console.log("Publicación enviada con éxito:", content);
  
    // Reinicia el estado del modal tras la publicación
  ...
  const handleAddPost = async (content, userId) => {
    if (!userId || !content || content.length === 0) {
      console.error("Error: userId o contenido no válido.");
      return;
    }
  
    try {
      const newPost = {
        content: content, // Aseguramos que el contenido sea un array válido
        userId, // Validamos que `userId` esté definido
      ...
      await createPost(newPost); // Guardamos la publicación en Firestore
