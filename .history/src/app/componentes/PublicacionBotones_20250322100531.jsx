  ...
  const handlePublish = (newComments) => {
    const newCommentThreads = newComments
      ...
      .map((comment) => ({
        ...
        userName: user?.displayName || "Usuario Anónimo", // Usa los datos del usuario desde la store
        userPhotoURL: user?.photoURL || "https://via.placeholder.com/50", // Foto del usuario desde la store
      }));
