...
const handleLogout = () => {
  ...
  signOut(auth)
    .then(() => {
      ...
      router.push("comp/Iniciosesion"); // Redirigir a la página de inicio de sesión
    })
    .catch((error) => {
  ...
  return (
    ...
    <ul className="flex flex-col items-center justify-center gap-7   ">
     ...
     <BiPlus /> {/* Icono para agregar una <IoHeartDislikeOutline />nueva publicación */}
      ...
      <CgDetailsMore
        className="relative hover:text-black w-12 h-12 flex items-center justify-center"
         onClick={handleLogout} // Al hacer clic, mostramos el segundo botón
      />
