...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handlePublish = async () => {
    if (!user?.id) {
      ...
      return; // No continuamos sin un userId válido
    }
    ...
    try {
      if (editingPost) {
        ...
        await updatePost(editingPost.id, { content }); // Actualiza en Firestore
        console.log("Publicación editada exitosamente:", content);
      } else {
        // Crear una nueva publicación
        await onAddPost(content, user.uid);
        console.log("Nueva publicación creada:", content);
      }
  ...
  useEffect(() => {
    console.log("Datos para editar:", editingPost); // Log para depurar datos
    if (editingPost) {
      ...
      setThreads(updatedThreads); // Precarga los datos del post en edición
      setIsPublishEnabled(true); // Habilita el botón "Publicar"
    } else {
      setThreads([
        { id: 1, active: true, placeholder: "¿Qué novedades tienes hoy?", content: "", files: [] },
      ]);
