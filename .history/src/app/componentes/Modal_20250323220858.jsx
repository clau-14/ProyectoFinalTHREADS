...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  ewThreads.some((thread) => thread.content.trim() !== "");
  ...
  const handlePublish = async () => {
    ...
    // Llamar a onAddPost para guardar el contenido en Firebase
    await onAddPost(content, user?.uid); // `user?.uid` es el ID único del usuario autenticado
    console.log("Contenido y archivos publicados:", content);
  
    // Reiniciar el estado para limpiar el modal después de publicar
    setThreads([
      { id: 1, active: true, placeholder: "¿Qué novedades tienes hoy?", content: "", files: [] }
    ]);
    setIsPublishEnabled(false);
    onClose(); // Cerrar el modal automáticamente después de publicar
  };
