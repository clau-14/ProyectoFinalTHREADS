  ...
  return (
    ...
    <ul className="flex flex-col items-center justify-center gap-7   ">
     ...
     <BiPlus /> {/* Icono para agregar una <IoHeartDislikeOutline />nueva publicación */}
      ...
      {/* Primer botón para mostrar el segundo */}
      <CgDetailsMore
        className="hover:scale-110 transition-transform"
        onClick={() => setShowLogoutButton(true)} // Mostrar el botón "Cerrar Sesión"
      />

      {/* Segundo botón para confirmar el cierre de sesión */}
      {showLogoutButton && (
        <button
          className="absolute top-16 bg-red-500 text-white py-2 px-4 rounded-md shadow-md hover:bg-red-600 transition-colors"
          onClick={handleLogout} // Cierra la sesión al hacer clic
        >
          Cerrar Sesión
        </button>
      )}
