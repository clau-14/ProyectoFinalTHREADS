  ...
  const manejarMeGusta = async () => {
    ...
    try {
      if (meGusta) {
        ...
        Error al manejar 'Me gusta':", error.message);
    } finally {
      setDeshabilitarsetLikesCount((prevCount) => prevCount + 1); // Actualiza la cuenta localmente
      }
      setMeGusta(!meGusta); // Cambia el estado del botón
    } catch (error) {
      console.error("(false); // Habilita el botón después de completar el evento
    }
  };
