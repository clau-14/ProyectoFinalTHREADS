...
import ModalButtons from './ModalBotones';


const Modal = ({ show, onClose, onPublish,selectedFiles , handleFileSelect}) => {
  if (!show) return null;
        ...
        {threads.map((thread, index) => (
          <section key={thread.id} className={`flex-1 ${thread.active ? '' : 'opacity-50 pointer-events-none'}`}>
            ...
            ></textarea>
             ...
             {(index === 0 || thread.showButtons) &&<ModalBotones onButtonClick={handleButtonClick} />}
          </section>
        ))}
            <ModalButtons onButtonClick={handleFileSelect} />
        {selectedFiles && Array.from(selectedFiles).map((file, index) => (
          <div key={index}>
            {file.type.startsWith('image/') && <img src={URL.createObjectURL(file)} alt="selected" />}
