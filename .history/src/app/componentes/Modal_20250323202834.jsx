...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handlePublish = async () => {
    ...
    const content = threads
      .filter((thread) => thread.content.trim() !== "")
      .map((thread) => ({
        text: thread.content.trim(),
        files: thread.files || [],
      }));
    ...
    try {
      for (const thread of content) {
        // Si hay archivos en el hilo, utiliza `addPostWithFile`
        if (thread.files.length > 0) {
          await addPostWithFile(thread.text, thread.files);
        } else {
          // De lo contrario, guarda solo el texto
          await handleAddPost([{ text: thread.text }], user.uid);
        }
      }
