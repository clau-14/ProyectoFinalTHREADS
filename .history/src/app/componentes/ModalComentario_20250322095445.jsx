...
import useUserStore from "@/store/userstore"; // Importa el store del usuario
import ModalComentario from "./ModalComentario"; // Importa el modal

const PublicacionBotones = ({ publication }) => {
  const { user } = useUserStore(); // Obtiene datos del usuario actual desde el store
  ...
  const handlePublish = (newComments) => {
    const newCommentThreads = newComments
      ...
      .map((comment) => ({
        ...
        text: comment.text,
        userName: user?.displayName || "Usuario Anónimo", // Obtiene el nombre del usuario desde el store
        userPhotoURL: user?.photoURL || "https://via.placeholder.com/50", // Obtiene la foto del usuario desde el store
        liked: false, // Nuevo estado para cada comentario
        likes: 0, // Contador inicial de "Me gusta" para cada comentario
      }));
