...
const Modal = ({ show, onClose, onPublish, editingPost, onAddPost  }) => {
  ...
  const handlePublish = async () => {
    if (!user?.uid) {
      console.error("Error: Usuario no autenticado. Deteniendo operación.");
      return; // Detenemos si el usuario no está autenticado
    }
  
    // Filtrar hilos con contenido no vacío
    const content = threads
      .filter((thread) => thread.content.trim() !== "") // Excluye textos vacíos
      .map((thread) => ({
        text: thread.content.trim(), // Elimina espacios innecesarios
        files: thread.files || [], // Incluye archivos seleccionados (si los hay)
      }));
  
    if (!content.length) {
      console.error("Error: No hay contenido válido para publicar.");
      return; // Detenemos si no hay contenido válido
    }
  
    try {
      await handleAddPost(content, user.uid); 
      console.log("Publicación creada exitosamente:", content);
  
      // Reiniciar el estado del modal después de publicar
  ...
  const handleAddPost = async (content, userId) => {
    console.log("Datos enviados a Firestore desde handleAddPost:", { content, userId });
  
    // Validación estricta de datos
    if (!userId || !content || content.some((thread) => !thread.text.trim())) {
      console.error("Error: userId o contenido inválido en handleAddPost.");
      return; // Detenemos si no hay datos válidos
    }
  
    try {
      const newPost = {
        content, // Array de hilos válidos
        userId, // ID del usuario autenticado
        timestamp: new Date(),
        likes: [], // Inicializamos los likes vacíos
      };
      await createPost(newPost.content, newPost.userId);
  ...
  console.log("Contenido enviado:", content);
