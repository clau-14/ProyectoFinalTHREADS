...
const ModalComentario = ({ show, onClose, onPublish, editingPost }) => {
  if (!show) return null;
  ...
  const [threads, setThreads] = useState([
    { id: 1, active: true, placeholder: "Agrega un tema..Responde a gettrads", content: "", files: [] },
  ...
  useEffect(() => {
    if (editingPost) {
      const updatedThreads = editingPost.content.map((item, index) => ({
        id: index + 1,
        active: true,
        placeholder: index === 0 ? "Agrega un tema..Responde a gettrads" : "¿Di algo más?",
        content: item.text || "",
        files: item.files || [],
      }));
