...
export default function ContenidoPage() {
  ...
  <div key={index} className="grouped-post">
    {/* Renderizar cada hilo como una sub-publicación */}
    {post.content.map((thread, threadIndex) => (
      <div key={threadIndex} className="sub-post flex items-start mb-4 p-4 border-b">
        ...
        <div className="ml-4">
          ...
          {thread.files?.map((file, fileIndex) => (
            <div key={fileIndex} className="mt-2">
              {file.type === "image" && (
                <img src={file.preview} alt={file.name} className="w-full h-auto rounded-lg" />
              )}
              {file.type === "video" && (
                <video src={file.preview} className="w-full h-auto border rounded-lg" controls />
              )}
            </div>
          ))}
        </div>
    ...
    {/* Menú desplegable para edición y eliminación */}
    <div className="ml-auto">
      ...
      {menuIndex === index && (
        <div className="absolute right-0 top-8 bg-white shadow-lg rounded-lg text-left p-2 w-48">
          <ul>
            <li onClick={() => handleEditPost(index)} className="hover:bg-gray-200 px-2 py-1 cursor-pointer">
              Editar publicación
            </li>
            <li onClick={() => handleDeletePost(index)} className="hover:bg-gray-200 px-2 py-1 cursor-pointer">
