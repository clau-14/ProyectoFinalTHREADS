...
export default function ContenidoPage() {
 ...
 const fetchPosts = async () => {
   try {
     const db = getFirestore(app); // Asegúrate de que `app` esté importado y correctamente inicializado
 ...
 const handleEditPost = (index) => {
  const postToEdit = publishedContent[index]; // Selecciona la publicación del estado
  if (!postToEdit || !postToEdit.id) {
    console.error("Error: La publicación seleccionada no contiene un ID válido.");
    return;
  }
  
  setEditingPost(postToEdit); // Asigna la publicación completa, incluyendo el `id`
