         ...
         <section style={{ overflowY: 'auto', msOverflowStyle: 'none', scrollbarWidth: 'none' }} 
           ...
           {publishedContent.map((post, index) => (
              ...
              alt="Foto de usuario"/>
                ...
                <div className='  '> < p>{post.content}</p></div>
                <div className="flex gap-2 mt-2">
                    {post.files.map((fileObj, fileIndex) => (
                      <div key={fileIndex}>
                        {fileObj.file.type.startsWith("image/") && (
                          <img src={fileObj.preview} alt="Publicado" className="w-24 h-24 rounded-lg" />
                        )}
                        {fileObj.file.type.startsWith("video/") && (
                          <video src={fileObj.preview} controls className="w-24 h-24 rounded-lg" />
                        )}
                      </div>
                    ))}
