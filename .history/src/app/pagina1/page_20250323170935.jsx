...
export default function ContenidoPage() {
   ...
   const handleAddPost = async (content, userId, files = []) => {
    if (!userId) {
      ...
      return; // Detenemos la ejecución si el userId no está disponible
    }
    ...
    try {
      ...
      const newPost = {
        content,
        files: uploadedFiles,
        userId, // Asegúrate de que esto tenga un valor válido
        timestamp: new Date(),
        likes: []
      };
      console.log("Datos preparados para Firestore:", newPost);
      await createPost(newPost); // Guardar en Firestore
