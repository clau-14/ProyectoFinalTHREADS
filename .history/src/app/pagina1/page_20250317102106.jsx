...
export default function ContenidoPage() {
  ...
  <div key={index} className="published-content flex items-start mb-8 p-4 bg-gray-100 rounded-xl relative">
    ...
    <div className="ml-4">
      ...
      <p>{post.content.map((item) => item.text).join(" ")}</p>
    </div>
    ...
    <div className="ml-auto relative">
      <button
        ...
        onClick={() => toggleMenu(index)} // Manejador para abrir/cerrar menú
      >
        ...
      </button>

      {menuIndex === index && ( // Solo muestra el menú si coincide el índice
            ...
            {/* Agrega más opciones aquí */}
                 ...
                 <div>
                   {post.content.map((item, i) => (
                     <div className='mt-20 -ml-32 mr-16 mb-20' key={i}>
                       <div className='-ml-2 -mt-2' dangerouslySetInnerHTML={{ __html: item.text }}></div>
                       {item.files?.map((file, fileIndex) => (
                         <div className='flex mt-5 p-1' key={fileIndex}>
                           {file.type === "image" && <img className='rounded-xl' src={file.preview} alt={file.name} />}
                           {file.type === "video" && <video className='border border-slate-300 p-5 rounded-xl w-full h-full' src={file.preview} controls />}
                         </div>
                       ))}
                     </div>
                   ))}
