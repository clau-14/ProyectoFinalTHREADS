       ...
       <div className='flex flex-col items-center justify-center h-full'>
         ...
         <section style={{ overflowY: 'auto', msOverflowStyle: 'none', scrollbarWidth: 'none' }} 
           ...
           {publishedContent.map((post, index) => (
              <div key={index} className="published-content flex flex-row items-center justify-center">
                <img className="rounded-full w-12 h-12 ml-8" src={post.userPhoto || ""} alt="Foto de usuario" />
                  ...
                  <div className='flex-col mt-20 -ml-32 mr-16 mb-20 border ' key={i}>
                  <div dangerouslySetInnerHTML={{ __html: item.text }}></div>
                    {item.files?.map((file, fileIndex) => (
                <div className='flex mt-5 p-10' key={fileIndex}>
                    {file.type === "image" && <img src={file.preview} alt={file.name} />}
                    {file.type === "video" && <video src={file.preview} controls />}
                    <button 
      className="text-red-500 font-bold mt-2 absolute right-10 top-20" 
