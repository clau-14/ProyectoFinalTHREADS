...
export const createPost = async (content, userId) => {
  console.log("Validando datos antes de guardar en Firestore:");
  ...
  if (!content || !userId || content.some((thread) => !thread.text.trim())) {
    ...
    return; // Detenemos si los datos no son válidos
  }
...
const cleanFirestoreDocument = async (docId) => {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      const validContent = data.content.filter((thread) => thread.text.trim() !== ""); // Filtrar hilos vacíos

      await updateDoc(docRef, { content: validContent });
      console.log("Documento actualizado correctamente:", docId);
    } else {
      console.log("El documento no existe:", docId);
    }
  } catch (error) {
    console.error("Error limpiando el documento:", error);
  }
};

export default cleanFirestoreDocument;
